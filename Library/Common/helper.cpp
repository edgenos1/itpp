...
namespace helper {
    ...
    cvec oversample(const cvec& symbols, const int j)
    {
        const int size = symbols.size(), half = size / 2;
        ...
        cvec signals(j * size);
        signals.zeros();
        signals.set_subvector(0, symbols.right(half));
        signals.set_subvector(signals.size() - half, symbols.left(half));
        return signals;
    }

    cvec downsample(const cvec& signals, const int j)
    {
        const int size = signals.size() / j, half = size / 2;

        it_assert_debug(is_power_of_2(size), "helper::oversample(): Data size must be power of 2.");
        it_assert_debug(is_power_of_2(j), "helper::oversample(): Data size must be power of 2.");

        cvec symbols(size);
        symbols.set_subvector(0, signals.right(half));
        symbols.set_subvector(size - half, signals.left(half));
        return symbols;
    }
